...
import com.wjj.geekfolder.homeclassify.DragGridAdapter
...
class CustomBehindView(private val mContext: Context, private val mCustomGroup: CustomGroup) : GridView(mContext) {
    ...
    override fun setAdapter(adapter: ListAdapter) {
        ...
        if (adapter is DragGridAdapter) {
            mDragAdapter = adapter as DragGridAdapter
        } else {
    ...
    private fun createFirstDragImage() {
        ...
        mWindowLayoutParams!!.flags = WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE or WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE
    ...
    private fun isTouchInItem(dragView: View?, x: Int, y: Int): Boolean {
        ...
        return if (y < topOffset || y > topOffset + dragView.height) {
            false
        } else true
    ...
    private fun animateReorder(oldPosition: Int, newPosition: Int) {
        ...
        if (isForward) {
            for (pos in oldPosition until newPosition) {
                ...
                if ((pos + 1) % mNumColumns == 0) {
                    resultList.add(createTranslationAnimations(view, (-view.width * (mNumColumns - 1)).toFloat(), 0f, view.height.toFloat(), 0f))
                } else {
        ...
        resultSet.setInterpolator(AccelerateDecelerateInterpolator())
